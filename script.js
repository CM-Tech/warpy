...
function randomThird(axis) {
    return axis / 2 + (min * (Math.floor(Math.random() * 3) - 1)) / 4;
}
class Bullet {
    constructor() {
        var wall = Math.random() < 0.5;
        this.x = wall ? c.width * Math.round(Math.random()) : Math.random() > (time>1050?0.9:0.25) ? randomThird(c.width) : you.x;
        this.y = !wall ? c.height * Math.round(Math.random()) : Math.random() > (time>1050?0.9:0.25) ? randomThird(c.height) : you.y;
...
function tick() {
    ...
    for (let b of bullets) {
        ...
        b.draw();
        if (running && (time<1000||time>1050) && b.x + 5 > you.x - 10 && b.y + 5 > you.y - 10 && b.x - 5 < you.x + 10 && b.y - 5 < you.y + 10) {
            running = false;
        }
    }
    ...
    if (!running) {
        ...
        ctx.fillStyle = "white";
        ctx.fillText("ontouchstart" in window ? "Tap to play again" : "Press space to play again", c.width / 2, c.height / 2);
    }
    if (running) time++;
